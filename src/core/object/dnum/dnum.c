...
struct Object* obj_dnum_new(const double val)
{
  ...
  LOG_DEBUG("creating the DNum object at address \"%p\" with value \"%lf\"",
            CAST(obj, void*),
            val);
  return obj;
}
...
{
  DASSERT(obj, "");
  LOG_DEBUG("removing the DNum object at address \"%p\" with value \"%lf\"",
            CAST(obj, void*),
            obj->data.dnum.val);
  xfree(obj);
}

struct Object* obj_dnum_new_from_str(const char* str)
{
  double val = str2double(str);
  return obj_dnum_new(val);
}

bool obj_dnum_is_true(const struct Object* obj)
{
                             ...
                             const struct Object* obj)
{
  ...
  switch (type) {
    ...
    case O_UNOP_LNOT:
      return (dbl_eq(val, 0.0)) ? CAST(&OBJ_FALSE, struct Object*) :
                                  CAST(&OBJ_TRUE, struct Object*);
    case O_UNOP_INCR: return obj_dnum_new(++val);
    case O_UNOP_DECR: return obj_dnum_new(--val);
    default: obj_err_set_unsupported_unop(type, obj->type); return NULL;
  }
}
