...
static struct DictionaryEntry* dict_slot_entry_new(void* key, void* val)
{
...
static NoRet dict_slot_entry_delete(struct Dictionary* dict,
                                    struct DictionaryEntry* entry)
{
  DASSERT(dict, "");
  DASSERT(entry, "");
...
static NoRet dict_slot_init(struct DictionarySlot* slot)
{
  slot->size = 0;
  slot->head = NULL;
  slot->tail = NULL;
...
static NoRet dict_slot_deinit(struct Dictionary* dict,
...
/* NOTE: This function does not handle the existance of entry with that key
 * already */
/*       so, you should probably do it YOURSELF */
static NoRet dict_slot_append(struct DictionarySlot* slot,
                              struct DictionaryEntry* entry)
{
  /* If slot is empty */
...
static bool dict_slot_remove(struct Dictionary* dict,
                             struct DictionarySlot* slot,
                             void* key)
{
  /* Obvious checks */
...
static struct DictionaryEntry* dict_slot_find(struct Dictionary const* dict,
                                              struct DictionarySlot* slot,
                                              const void* key)
{
...
NoRet dict_init(struct Dictionary* dict,
                uint32 size,
                ...
                NoRet (*val_free_f)(void*))
{
  ...
  for (uint32 i = 0; i < size; ++i) dict_slot_init(&dict->slots[i]);
...
struct Dictionary* dict_new(uint32 size,
                            uint32 (*hash_f)(const void*),
                            int (*key_cmp_f)(const void*, const void*),
                            NoRet (*key_free_f)(void*),
                            NoRet (*val_free_f)(void*))
{
  ...
  dict_init(dict, size, hash_f, key_cmp_f, key_free_f, val_free_f);
  return dict;
}
...
bool dict_insert_raw(struct Dictionary* dict,
                     void* key,
                     void* val,
                     int64 existing_hash)
{
  ...
  uint32 i;
  i = (existing_hash == -1) ? dict_get_key_index(dict, key) :
...
bool dict_insert(struct Dictionary* dict, void* key, void* val)
{
  return dict_insert_raw(dict, key, val, -1);
}

NoRet dict_replace(struct Dictionary* dict, void* key, void* val)
{
  /* Retreive the index of a slot */
...
bool dict_remove(struct Dictionary* dict, void* key)
{
  /* Retreive the index of a slot */
  uint32 i = dict_get_key_index(dict, key);
  /* Retreive the slot */
...
void* dict_find_raw(struct Dictionary const* dict,
                    const void* key,
                    int64 existing_hash)
{
  /* Retreive the index of a slot to find in */
  uint32 i =
    (existing_hash == -1) ? dict_get_key_index(dict, key) : existing_hash;
...
void* dict_find(struct Dictionary const* dict, const void* key)
{
...
#define DICT_BENCH_START(msg) gettimeofday(&tval_before, NULL);
...
#define DICT_BENCH_DEINIT() dict_deinit(&dict)
...
static NoRet dict_test_uniqueness(void)
{
  ...
  for (uint64 i = 0; i < dict.size; ++i) {
    for (uint64 j = 0; j < dict.slots[i].size; ++j) printf("#");
    printf("\n");
  }
