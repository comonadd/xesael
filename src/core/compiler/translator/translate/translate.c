...
bool translate_node(struct Translator* T, struct ASTNode* node)
{
  ...
  if (ast_node_is_lit(node)) return translate_lit_node(node);

  /* If node is a variable */
  if (node->type == AST_NODE_VAR) return translate_var_node(node);

  /* If node is a assignment */
  if (node->type == AST_NODE_ASSIGN) return translate_assign_node(T, node);

  /* If node is a binary operator */
  if (ast_node_is_binop(node)) return translate_binop_node(node);

  /* If node is a unary operator */
  if (ast_node_is_unop(node)) return translate_unop_node(node);

  switch (node->type) {
    case AST_NODE_BLOCK: return translate_nodes(node->data.block.stmts);
    case AST_NODE_IF: return translate_if_node(node);
    case AST_NODE_WHILE: return translate_while_node(node);
    case AST_NODE_IMPORT: return translate_import_node(node);
    case AST_NODE_VAR_DEF: return translate_var_def_node(node);
    case AST_NODE_CONST_DEF: return translate_const_def_node(node);
    case AST_NODE_FUNC_DEF: return translate_func_def_node(node);
    case AST_NODE_ENUM_DEF: return translate_enum_def_node(node);
    case AST_NODE_CALL: return translate_call_node(node);
...
bool translate_nodes(struct ByteCode* output, struct XVector* nodes)
{
  ...
  for (uint64 i = 0; i < nodes->size; ++i) {
    ...
    /* Compile the node */
    if (!__translate_node(T, node)) return false;
...
bool translate(struct ByteCode* output, struct AST* ast)
{
  ...
  for (uint64 i = 0; i < nodes->size; ++i) {
    /* Translate the node */
    if (!__translate_node(xvec_get(nodes, i))) goto out;
    /* Free the registers */
    translator_free_regs(&T);
  }
