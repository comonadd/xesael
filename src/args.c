...
static bool __arg_action_show_version(
    const uint8 i,
    struct Arguments const* args);

static bool __arg_action_show_help(
    const uint8 i,
    struct Arguments const* args);

static bool __arg_action_set_optimization_lvl(
    const uint8 i,
    struct Arguments const* args);

static bool __arg_action_set_path(
    const uint8 i,
    struct Arguments const* args);

static bool __arg_action_set_output_path(
    const uint8 i,
    struct Arguments const* args);

static bool __arg_action_include(
    const uint8 i,
    struct Arguments const* args);

static bool __arg_action_compile(
    const uint8 i,
    struct Arguments const* args);

static bool __arg_action_execute(
    const uint8 i,
    struct Arguments const* args);

static bool __arg_action_execute_str(
    const uint8 i,
    struct Arguments const* args);
...
#define append_include(___include) do {                 \
        if (includes_count >= INCLUDES_MAX_COUNT)       \
            goto out_too_many_includes;                 \
    ...
    for (uint64 j = 0; j < strlen(includes_s); ++j) {
        switch (includes_s[j]) {
            case ',':
                ...
                break;
            default:
                include[include_len++] = includes_s[j];
        }
    }
    ...
    struct Arguments const* args)
{
    ...
    char const* output_path = __res->output_path ? __res->output_path : DEFAULT_OUTPUT_PATH;
    dump(bc, output_path);
        ...
        if ((arg[0] == '-') && (arg[1] == '-')) {
            ...
            for (uint64 j = 0; j < __ARGS_ACTIONS_COUNT; ++j) {
                if (STREQ(__ARGS_ACTIONS[j].l, arg)) {
                    if (!__ARGS_ACTIONS[j].action(j, args))
                        return NULL;
                }
            }
        } else if (arg[0] == '-') {
