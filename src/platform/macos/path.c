...
char* path_concat(char const* p1, char const* p2)
{
  ...
  if (!(p1 && p2)) {
    err_set_unknown();
    LOG_DEBUG("wrong inputs, bool(p1) == %d, bool(p2) == %d", !!p1, !!p2);
    return NULL;
  }
...
char* path_get_base(char const* path)
{
  ...
  if (path[path_len - 1] == PLATFORM_PATH_SEPARATOR_CH) return NULL;
...
char* path_concat_with_ext(char const* path, char const* ext)
{
  ...
  if (!(path && ext)) {
    LOG_DEBUG("wrong inputs, bool(path) == %d, bool(ext) == %d", !!path, !!ext);
    err_set_unknown();
    /* Fail */
    return NULL;
  }
...
bool get_path_access_mode(char const* path, enum PathAccessMode* mode)
{
  if (!path) {
    err_set_unknown();
    LOG_DEBUG("wrong input, path is NULL");
  ...
  switch (stat_.st_mode) {
    case 0700:
    case 1700: *mode = PAM_USER_ONLY; break;
    case 0777:
    case 1777: *mode = PAM_ALL; break;
    default:
      err_set_unknown();
                 ...
                 const bool dir)
{
  ...
  if (res == -1) {
    switch (errno) {
      default:
        LOG_DEBUG(
          "not handled \"errno\" case switch statement, strerror(errno) == "
          "\"%s\"",
          strerror(errno));
        err_set_unknown();
    }
