...
bool dlinkedlist_is_empty(struct DLinkedList* list)
{
...
{
  return (list->tail != list->sentinel) ? list->tail : list->head;
}
...
NoRet dlinkedlist_append(struct DLinkedList* list, struct DLinkedListNode* node)
{
  if (dlinkedlist_is_empty(list)) {
    list->head       = node;
    list->head->prev = list->sentinel;
...
NoRet dlinkedlist_remove(struct DLinkedList* list, struct DLinkedListNode* node)
{
