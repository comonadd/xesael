...
NoRet bench_lex_all_separately(void)
{
    ...
    BENCH_LEXER_BENCHMARK_END(
        "lex all tokens, each separately, %u time(s)",
        ITER_COUNT);
}
...
NoRet bench_lex_all_together(void)
{
    ...
    BENCH_LEXER_BENCHMARK_END(
        "lex all tokens, together, %u time(s)",
        ITER_COUNT);
    xfree(all_tokens);
}
    ...
    const bool cond)
{
    ...
    assert(xfile_load(&file, &file_contents,
                      &file_contents_size));

    toks = lex(file_contents);
    if (cond) assert(err_is_ok());
    else assert(!(toks || err_is_ok()));
