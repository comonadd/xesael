...
struct Object* obj_binop_cmp_inum_dnum(
    const enum ObjectBinOpType type,
    ...
    const struct Object* rop)
{
    ...
    switch (type) {
        ...
        case O_BINOP_LAND: return (obj_dnum_is_true(lop) && obj_dnum_is_true(rop)) ? tobj : fobj;
        case O_BINOP_LOR: return (obj_dnum_is_true(lop) || obj_dnum_is_true(rop)) ? tobj : fobj;
        default: IMPOSSIBLE();
    }
...
struct Object* obj_binop_cmp_inum(
    const enum ObjectBinOpType type,
    const struct Object* lop,
    const struct Object* rop)
{
...
struct Object* obj_binop_cmp_dnum(
    const enum ObjectBinOpType type,
    const struct Object* lop,
    const struct Object* rop)
{
...
struct Object* obj_binop_cmp_str(
    const enum ObjectBinOpType type,
    const struct Object* lop,
    const struct Object* rop)
{
...
struct Object* obj_binop_arith_inum_dnum(
    const enum ObjectBinOpType binop,
    const struct Object* lop,
    const struct Object* rop)
{
    /* Obvious checks */
...
struct Object* obj_binop_arith_inum_str(
    const enum ObjectBinOpType binop,
    const struct Object* lop,
    const struct Object* rop)
{
    ...
    switch (binop) {
        case O_BINOP_MUL:
            return obj_str_new(str_mul(rop_val, rop_len, lop_val));
        default:
            obj_err_set_unsupported_binop(binop, lop->type, rop->type);
            return NULL;
    }
...
struct Object* obj_binop_arith_inum(
    const enum ObjectBinOpType binop,
    const struct Object* lop,
    const struct Object* rop)
{
...
struct Object* obj_binop_arith_dnum(
    const enum ObjectBinOpType binop,
    const struct Object* lop,
    const struct Object* rop)
{
...
struct Object* obj_binop_arith_str(
    const enum ObjectBinOpType binop,
    const struct Object* lop,
    const struct Object* rop)
{
    ...
    switch (rop->type) {
        ...
        case OBJT_STR: {
            ...
            switch (binop) {
                case O_BINOP_ADD:
                    return obj_str_new(str_concat(lop_val, rop_val));
    ...
    const struct Object* rop)
{
    ...
    switch (binop) {
        case O_BINOP_LAND:
            return (obj_is_true(lop) && obj_is_true(rop)) ?
                tobj : fobj;
        case O_BINOP_LOR:
            return (obj_is_true(lop) || obj_is_true(rop)) ?
                tobj : fobj;
    ...
    const struct Object* rop)
{
    ...
    if (obj_binop_is_bool(binop))
        return obj_binop_bool(binop, lop, rop);

    /* If given binary operaetion is a arithmetic operation */
    if (obj_binop_is_arith(binop))
        return obj_binop_arith(binop, lop, rop);
