        ...
        switch (node->type) {
            case AST_NODE_BLOCK:
                if (node->data.block.stmts)
                    xvec_delete(node->data.block.stmts);
                break;
            ...
            case AST_NODE_IF:
                ...
                if (node->data.if_.else_body)
                    ast_node_delete(node->data.if_.else_body);
                break;
            ...
            case AST_NODE_RETURN:
                ast_node_delete(node->data.ret.val);
                break;

            case AST_NODE_IMPORT:
                obj_decr_ref_count(node->data.import.val);
                break;
...
enum ASTNodeType tt_unop2astt(const enum TokenType type)
{
    ...
    return CAST(
        AST_NODE_UNOP_FIRST + type - TOK_UNOP_FIRST,
        enum ASTNodeType);
}
...
enum ASTNodeType tt_binop2astt(const enum TokenType type)
{
    ...
    return CAST(
        AST_NODE_BINOP_FIRST + type - TOK_BINOP_FIRST,
        enum ASTNodeType);
}
...
enum ASTNodeType tt_assign_shorthand2astt(const enum TokenType type)
{
    ...
    return CAST(
        AST_NODE_BINOP_FIRST + type - TOK_ASSIGN_SHORTHAND_FIRST,
        enum ASTNodeType);
}
...
struct Object* ast_node_lit_get_obj(struct ASTNode* node)
{
    ...
    switch (node->type) {
        case AST_NODE_STRLIT:
            return node_val;
        case AST_NODE_INUMLIT:
            return obj_inum_new_from_str(node_val->data.str.val);
        case AST_NODE_DNUMLIT:
            return obj_dnum_new_from_str(node_val->data.str.val);
        default:
            IMPOSSIBLE();
            /* Fail */
            return false;
...
struct ASTNode* mk_ast_node_lit(
    const enum ASTNodeType type,
    const LineNumber ln,
    struct Object* val)
{
...
struct ASTNode* mk_ast_node_var(
    const LineNumber ln,
    struct Object* name)
{
    DASSERT(name, "");
...
struct ASTNode* mk_ast_node_unop(
    const LineNumber ln,
    const enum ASTNodeType type,
    struct ASTNode* op)
{
...
struct ASTNode* mk_ast_node_binop(
    const LineNumber ln,
    const enum ASTNodeType type,
    struct ASTNode* lop,
    struct ASTNode* rop)
...
struct ASTNode* mk_ast_node_block(
    const LineNumber ln,
...
struct ASTNode* mk_ast_node_while(
    const LineNumber ln,
    struct ASTNode* cond,
    struct ASTNode* body)
{
...
struct ASTNode* mk_ast_node_return(
    const LineNumber ln,
    struct ASTNode* val)
{
...
struct ASTNode* mk_ast_node_import(
    const LineNumber ln,
    struct Object* name)
{
...
struct ASTNode* mk_ast_node_func_def(
    const LineNumber ln,
    struct ASTNode* body,
    struct Object* name,
    struct XVector* args)
...
struct ASTNode* mk_ast_node_var_def(
    const LineNumber ln,
    struct Object* name,
...
struct ASTNode* mk_ast_node_const_def(
    const LineNumber ln,
    struct Object* name,
    struct ASTNode* initializator)
{
...
struct ASTNode* mk_ast_node_call(
    const LineNumber ln,
    struct Object* name,
    struct XVector* args)
{
