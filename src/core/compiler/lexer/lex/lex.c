...
static NoRet lex_plus_ch(void)
{
  ...
  switch (lexer_get()) {
    ...
    case EQ_CH:
      ...
      break;
    default:
      if (tok_is_binop(lexer_get_pt()) || tok_is_sep(lexer_get_pt()) ||
          tok_is_invalid(lexer_get_pt())) {
        /* Don't do anything (we don't need unary '+') */
      } else
        lexer_append_tok(&RTOKS[TOK_ADD]);
  }
}
  ...
  switch (lexer_get()) {
    ...
    default:
      if (tok_is_binop(lexer_get_pt()) || tok_is_sep(lexer_get_pt()) ||
        ...
        lexer_append_tok(&RTOKS[TOK_SUB]);
  }
}
...
/**
   ...
   #   of type type_else
**/
static NoRet read_rtok1(const char ch1,
                        ...
                        const enum TokenType type_else)
{
  ...
  if (lexer_get() == ch1) {
    ...
    lexer_append_tok(&RTOKS[type_else]);
}
...
struct XVector* lex(char const* text)
{
  ...
  while (true) {
    ...
    /* (which means that there is no possibility that it represents something
     * another) */
    /* and appends the reserved token into the array of tokens depending on the
     * character */
    {
    ...
    switch (ch) {
      ...
      case STAR_CH:
        if (lexer_lookahead_eq(STAR_CH)) {
          ...
          read_rtok1(EQ_CH, TOK_ASSIGN_MUL, TOK_MUL);
        break;
      ...
      case LT_CH:
        if (lexer_lookahead_eq(LT_CH)) {
          ...
          read_rtok1(EQ_CH, TOK_LTE, TOK_LT);
        break;
      ...
      case GT_CH:
        if (lexer_lookahead_eq(GT_CH)) {
          ...
          read_rtok1(EQ_CH, TOK_LTE, TOK_GT);
        break;
      ...
      case PIPE_CH:
        if (lexer_lookahead_eq(PIPE_CH)) {
          ...
          read_rtok1(EQ_CH, TOK_ASSIGN_BOR, TOK_BOR);
        break;
      ...
      case AMP_CH:
        if (lexer_lookahead_eq(AMP_CH)) {
          ...
          read_rtok1(EQ_CH, TOK_ASSIGN_BAND, TOK_BAND);
        break;

      case CARET_CH: read_rtok1(EQ_CH, TOK_ASSIGN_BXOR, TOK_BXOR); break;
      case HASH_CH:
        if (!lex_comment()) goto out;
        continue;
      case QUOTE_CH:
        if (!lex_strlit()) goto out;
        continue;
      case STR_END_CH:
      case EOF_CH: goto done;
