...
NoRet bytecode_init(struct ByteCode* bc)
{
    ...
    bc->instructions = xmalloc(
        BYTECODE_INIT_CAPACITY *
        sizeof(Instruction));
}
    ...
    const Instruction instr)
{
    if (bc->instructions_count >= bc->instructions_capacity) {
        ...
        bc->instructions = xrealloc(
            bc->instructions,
            bc->instructions_capacity * sizeof(Instruction));
    ...
    const Instruction instr)
{
    if ((pos + 1) >= bc->instructions_capacity) {
        ...
        bc->instructions = xrealloc(
            bc->instructions,
            bc->instructions_capacity * sizeof(*bc->instructions));
