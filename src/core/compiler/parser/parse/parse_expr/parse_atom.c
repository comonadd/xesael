...
static struct ASTNode* parse_atom_call(struct Token const* tok)
{
  ...
  while (true) {
    ...
    switch (arg_tok->type) {
      case TOK_SEP_COMMA: parser_skip(); break;
      case TOK_SEP_RPAREN: parser_skip(); goto done;
...
struct ASTNode* parse_atom(void)
{
  ...
  switch (tok->type) {
    case TOK_INUMLIT:
      res = mk_ast_node_lit(AST_NODE_INUMLIT, parser_get_ln(), tok->val);
      break;

    case TOK_DNUMLIT:
      res = mk_ast_node_lit(AST_NODE_DNUMLIT, parser_get_ln(), tok->val);
      break;

    case TOK_STRLIT:
      res = mk_ast_node_lit(AST_NODE_STRLIT, parser_get_ln(), tok->val);
      break;

    case TOK_KEYWORD_TRUE:
      res = mk_ast_node_lit(
        AST_NODE_BOOLLIT, parser_get_ln(), CAST(&OBJ_TRUE, struct Object*));
      break;

    case TOK_KEYWORD_FALSE:
      res = mk_ast_node_lit(
        AST_NODE_BOOLLIT, parser_get_ln(), CAST(&OBJ_FALSE, struct Object*));
      break;
    ...
    case TOK_ID: {
      ...
      res = mk_ast_node_var(parser_get_ln(), tok->val);
      break;
    }
    ...
    default: {
      compiler_err_setf(
        parser_get_ln(), "expression expected, got \"%s\"", tok2str(tok));
      return NULL;
    }
  }
  ...
  if (tok_is_postfix_unop(tok)) {
    ...
    /* Make a new unary operator node */
    res = mk_ast_node_unop(parser_get_ln(), tt_unop2astt(tok->type), res);
  }
