...
bool file_copy(const char* src_path, const char* dst_path)
{
  ...
  for (;;) {
    ...
    if (res == -1) {
      switch (errno) {
        default:
          LOG_DEBUG("not handled \"errno\" in switch on read (errno == %s)",
                    strerror(errno));
          err_set_unknown();
      }
    ...
    if (res == -1) {
      switch (errno) {
        default:
          LOG_DEBUG("not handled \"errno\" in switch on write (errno == %s)",
                    strerror(errno));
          err_set_unknown();
      }
...
bool dir_copy(const char* src_path, const char* dst_path)
{
  ...
  if (!get_path_access_mode(src_path, &src_access_mode)) return false;
  if (!path_create(dst_path, src_access_mode, true)) return false;
  ...
  while (entries_count != 0) {
    ...
    if (!res) {
      ...
      for (; entries_count > 0; --entries_count) xfree(entries[entries_count]);
