...
const struct Object OBJ_STR_EMPTY = {
  .data = {.str = {0, NULL}}, .ref_count = 1, .type = OBJT_STR};
...
{
  dict_init(&str_objs, STR_OBJS_DICT_SIZE, NULL, NULL, NULL, NULL);
}
...
NoRet obj_str_delete(struct Object* obj)
{
  /* Obvious check */
  DASSERT(obj, "");
  LOG_DEBUG("removing string object at \"%p\" with value \"%s\"",
            (void*)obj,
            obj->data.str.val);
  /* Obvious check */
                            ...
                            const struct Object* obj)
{
  ...
  if (type == O_UNOP_LEN) return obj_inum_new(obj->data.str.len);
