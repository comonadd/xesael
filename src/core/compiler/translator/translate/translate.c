    ...
    struct ASTNode* node)
{
    ...
    if (ast_node_is_lit(node))
        return translate_lit_node(node);

    /* If node is a variable */
    if (node->type == AST_NODE_VAR)
        return translate_var_node(node);

    /* If node is a assignment */
    if (node->type == AST_NODE_ASSIGN)
        return translate_assign_node(T, node);

    /* If node is a binary operator */
    if (ast_node_is_binop(node))
        return translate_binop_node(node);

    /* If node is a unary operator */
    if (ast_node_is_unop(node))
        return translate_unop_node(node);
