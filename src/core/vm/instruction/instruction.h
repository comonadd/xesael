...
#define INSTR_CODE_IS_UNOP(code) \
  ((code >= IOP_UNOP_FIRST) && (code <= IOP_UNOP_LAST))
#define INSTR_CODE_IS_BINOP(code) \
  ((code >= IOP_BINOP_FIRST) && (code <= IOP_BINOP_LAST))
...
#define binop_instr_code_from_ast_node(___node)                               \
  CAST(CAST((___node)->type - AST_NODE_BINOP_FIRST, enum InstructionOpCode) + \
         IOP_BINOP_FIRST,                                                     \
       enum InstructionOpCode)

#define unop_instr_code_from_ast_node(___node)                               \
  CAST(CAST((___node)->type - AST_NODE_UNOP_FIRST, enum InstructionOpCode) + \
         IOP_UNOP_FIRST,                                                     \
       enum InstructionOpCode)
...
#define INSTR_ASSERT_CODE(___instr, ___code) \
  DASSERT(I_OP_GET(___instr) == (___code), "")

/***********/
/* Helpers */
/***********/

char const* instr_code2str(const enum InstructionOpCode code);
...
Instruction instr_new(const enum InstructionOpCode code);

Instruction instr_mov_new(const RegisterID rid1, const RegisterID rid2);

Instruction instr_load_new(const ImmediateID immediate_id,
                           const RegisterID rid);

Instruction instr_varset_new(const ImmediateID var_name_immediate_id,
                             const RegisterID val_rid);

Instruction instr_varload_new(const ImmediateID var_name_immediate_id,
                              const RegisterID dst_rid);

/* Relative jump */
Instruction instr_jmpr_new(const InstructionOffset offset);

Instruction instr_jmprc_new(const InstructionOffset offset,
                            const RegisterID rid);

Instruction instr_jmprci_new(const InstructionOffset offset,
                             const RegisterID rid);

/* Absolute jump */
Instruction instr_jmpa_new(const InstructionPosition pos);

Instruction instr_jmpac_new(const InstructionPosition pos,
                            const RegisterID cond_rid);

Instruction instr_jmpaci_new(const InstructionPosition pos,
                             const RegisterID cond_rid);

/* Calling */
Instruction instr_call_new(const ImmediateID func_name_immediate_id,
...
/* Unary operators */
Instruction instr_unop_new(const enum InstructionOpCode code,
                           const RegisterID src_rid,
                           const RegisterID dst_rid);

/* Binary operators */
Instruction instr_binop_new(const enum InstructionOpCode code,
                            const RegisterID src1_rid,
                            const RegisterID src2_rid,
                            const RegisterID dst_rid);

Instruction instr_import_new(const ImmediateID name_immediate_id);
