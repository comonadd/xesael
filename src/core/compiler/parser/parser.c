...
bool parser_eat_sep_or_end(void)
{
    ...
    if (!tok_is_sep(tok) && !tok_is_invalid(tok)) {
        ...
        compiler_err_setf(
            parser_get_ln(),
            "separator or EOF expected, got \"%s\"",
            tok2str(tok));
        /* Fail */
        return false;
    }
...
static NoRet TEST_PARSE_CONDITION(
    ...
    const bool cond)
{
    ...
    if (cond) assert(err_is_ok());
    else assert(!(res || err_is_ok()));
...
NoRet test_parser(void)
{
    ...
    TEST_PARSE_CONDITION("bad compound expressions", "bad/expr_compound.gshl", false);
    TEST_PARSE_CONDITION("good compound expressions", "good/expr_compound.gshl", true);

    TEST_PARSE_CONDITION("bad variable definition statement", "bad/var_def.gshl", false);
    TEST_PARSE_CONDITION("good variable definition statement", "good/var_def.gshl", true);

    TEST_PARSE_CONDITION("bad const definition statement", "bad/const_def.gshl", false);
    TEST_PARSE_CONDITION("good const definition statement", "good/const_def.gshl", true);
