...
static NoRet ast_nodes_print_(
    struct XVector const* nodes,
    const uint64 indentation_level);

static NoRet ast_node_print_(
...
static char* ast_node_lit_val2str(struct ASTNode const* node)
{
    if (node->type == AST_NODE_BOOLLIT) {
        if (node->data.lit.val == &OBJ_TRUE) return "TRUE";
        else return "FALSE";
    } else return node->data.lit.val->data.str.val;
}
        ...
        switch (node->type) {
            ...
            case AST_NODE_IF:
                ...
                for (uint64 i = 0; i < node->data.if_.stmts->size; i += 2) {
                    ast_node_print_(xvec_get(node->data.if_.stmts, i), indentation_level + 2);
                    ast_node_print_(xvec_get(node->data.if_.stmts, i + 1), indentation_level + 2);
                }
                printf("%s%s}\n", indentation_per_level, indentation);
            ...
            case AST_NODE_VAR_DEF:
                ...
                if (node->data.var_def.initializator) {
                    printf(
                        ...
                        indentation_per_level,
                        indentation);
                    ast_node_print_(node->data.var_def.initializator, indentation_level + 1);
                }
                break;
            ...
            case AST_NODE_CONST_DEF:
                ...
                printf(
                    ...
                    indentation_per_level,
                    indentation);
                ast_node_print_(node->data.const_def.initializator, indentation_level + 1);
                break;
