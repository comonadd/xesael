...
{
  return (tok->type == TOK_SEP_NEWLINE) || (tok->type == TOK_SEP_SEMI);
}

bool tok_can_start_def_stmt(struct Token const* tok)
{
  return (tok->type == TOK_KEYWORD_VAR) || (tok->type == TOK_KEYWORD_CONST) ||
         (tok->type == TOK_KEYWORD_ENUM) || (tok->type == TOK_KEYWORD_FUNC);
}

bool tok_can_start_flow_stmt(struct Token const* tok)
{
  return (tok->type == TOK_KEYWORD_BREAK) ||
...
{
  return (tok->type == TOK_KEYWORD_FOR) || (tok->type == TOK_KEYWORD_WHILE) ||
