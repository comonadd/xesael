...
struct Immediate* immediate_list_insert(
    ImmediateList* list,
    char const* name,
    struct Object* obj)
{
    struct Immediate* I = bst_find(
        CAST(list, struct BST*),
        name);
...
struct Immediate* immediate_list_lookup(
    ImmediateList const* list,
    char const* name)
{
...
static NoRet immediate_list_pack_node_f(
    struct BSTNode* node)
{
...
struct Object* const* immediate_list_pack(
    ImmediateList const* list)
{
    DASSERT(list, "");
    if (list->size > 0) {
