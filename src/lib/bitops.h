...
#define RESET_BITS(bits, pos, n) do {				\
	for (uint8 i = 0; i < n; ++i) reset_bit(bits, pos + i); \
    } while (0)

#define SET_BITS(___bits, ___pos, ___n, ___bits_to_set) do {		\
...
#define PRINTN_BITS(bits, n) do {					\
	uint64 __bits = CAST(bits, uint64);				\
	for (int32 i = n; i >= 0; --i) putc(!!GET_BIT(__bits, i - 1) + '0', stdout); \
	putc('\n', stdout);						\
    } while (0)
